...
#include "DJ01PlayerStart.generated.h"

class AController;
class UObject;

enum class EDJ01PlayerStartLocationOccupancy
{
	Empty,
	Partial,
	Full
...
*/
UCLASS(Config = Game)
class DJ01_API ADJ01PlayerStart : public APlayerStart
{
	GENERATED_BODY()

public:
	ADJ01PlayerStart(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	const FGameplayTagContainer& GetGameplayTags() { return StartPointTags; }

	EDJ01PlayerStartLocationOccupancy GetLocationOccupancy(AController* const ControllerPawnToFit) const;
