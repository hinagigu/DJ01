...
#include "DJ01/Camera/Public/DJ01CameraMode_ThirdPerson.h"
#include "DJ01/Camera/Public/DJ01CameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "DJ01/Camera/Public/DJ01PenetrationAvoidanceFeeler.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DJ01CameraMode_ThirdPerson)

namespace DJ01CameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UDJ01CameraMode_ThirdPerson::UDJ01CameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FDJ01PenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FDJ01PenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FDJ01PenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FDJ01PenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FDJ01PenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FDJ01PenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FDJ01PenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UDJ01CameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UDJ01CameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IDJ01CameraAssistInterface* TargetControllerAssist = Cast<IDJ01CameraAssistInterface>(TargetController);

	IDJ01CameraAssistInterface* TargetActorAssist = Cast<IDJ01CameraAssistInterface>(TargetActor);
	...
	IDJ01CameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IDJ01CameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IDJ01CameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IDJ01CameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UDJ01CameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IDJ01CameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FDJ01PenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(DJ01CameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UDJ01CameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UDJ01CameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
