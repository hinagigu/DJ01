...
#include "DJ01/AbilitySystem/Public/DJ01GameplayCueManager.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DJ01GameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EDJ01EditorLoadMode
{
...
namespace DJ01GameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("DJ01.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via DJ01GameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UDJ01GameplayCueManager::DumpGameplayCues));

	static EDJ01EditorLoadMode LoadMode = EDJ01EditorLoadMode::LoadUpfront;
}
...
UDJ01GameplayCueManager::UDJ01GameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UDJ01GameplayCueManager* UDJ01GameplayCueManager::Get()
{
	return Cast<UDJ01GameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UDJ01GameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UDJ01GameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogDJ01, Warning, TEXT("UDJ01GameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UDJ01GameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (DJ01GameplayCueManagerCvars::LoadMode)
	{
	case EDJ01EditorLoadMode::LoadUpfront:
		return true;
	case EDJ01EditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EDJ01EditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UDJ01GameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UDJ01GameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UDJ01GameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UDJ01GameplayCueManager* GCM = Cast<UDJ01GameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogDJ01, Error, TEXT("DumpGameplayCues failed. No UDJ01GameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogDJ01, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogDJ01, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogDJ01, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogDJ01, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogDJ01, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogDJ01, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogDJ01, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogDJ01, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogDJ01, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogDJ01, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogDJ01, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogDJ01, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UDJ01GameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UDJ01GameplayCueManager* StrongThis = Get())
					{
...
void UDJ01GameplayCueManager::HandlePostGarbageCollect()
{
...
void UDJ01GameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogDJ01, Warning, TEXT("UDJ01GameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UDJ01GameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (DJ01GameplayCueManagerCvars::LoadMode)
	{
	case EDJ01EditorLoadMode::LoadUpfront:
...
void UDJ01GameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UDJ01GameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UDJ01GameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UDJ01GameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UDJ01GameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UDJ01GameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
