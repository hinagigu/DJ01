...
// Description: ReSources 绑定集 - 资源相关属性
//
// Tag Bindings: 0
...
#define DJ01_BINDING_SET_RE_SOURCES_CALLBACKS() \
	...
	void OnMaxHealthChanged(const FOnAttributeChangeData& Data) { if (auto* ASC = BindingSet_ReSources_ASC.Get()) { if (const auto* Set = ASC->GetSet<UDJ01ResourceSet>()) { MaxHealth = Set->GetTotalMaxHealth(); } } } \
	...
	void OnMaxManaChanged(const FOnAttributeChangeData& Data) { if (auto* ASC = BindingSet_ReSources_ASC.Get()) { if (const auto* Set = ASC->GetSet<UDJ01ResourceSet>()) { MaxMana = Set->GetTotalMaxMana(); } } }
...
// 初始化值宏 - 内部使用
// ============================================================
#define DJ01_BINDING_SET_RE_SOURCES_INIT_VALUES(InASC) \
	const UDJ01ResourceSet* BindingSet_ReSources_ResourceSet = InASC->GetSet<UDJ01ResourceSet>(); \
	...
	MaxHealth = BindingSet_ReSources_ResourceSet ? BindingSet_ReSources_ResourceSet->GetTotalMaxHealth() : 0.0f; \
	...
	MaxMana = BindingSet_ReSources_ResourceSet ? BindingSet_ReSources_ResourceSet->GetTotalMaxMana() : 0.0f;
