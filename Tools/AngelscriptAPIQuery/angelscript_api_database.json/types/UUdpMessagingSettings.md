...
### EnabledByDefault
- **类型**: `bool`
- **描述**: Whether UDP messaging is enabled by default. If false -messaging will need to be added
...
### EnableTransport
- **类型**: `bool`
- **描述**: Whether the UDP transport channel is enabled.
Can be specified on the command line with `-UDPMESSAGING_TRANSPORT_ENABLE=`

### bAutoRepair
- **类型**: `bool`
- **描述**: Whether the UDP transport channel should try to auto repair when in error.

### MaxSendRate
- **类型**: `float32`
- **描述**: Maximum sustained transmission rate in Gbit / s.
...
- **类型**: `uint`
- **描述**: The number of consecutive attempt the auto repair routine will try to repair.

### WorkQueueSize
- **类型**: `uint16`
- **描述**: The buffer size for the working send queues. Each node connections
...
### UnicastEndpoint
- **类型**: `FString`
- **描述**: The IP endpoint to listen to and send packets from.
...
### MulticastEndpoint
- **类型**: `FString`
- **描述**: The IP endpoint to send multicast packets to.
...
### MessageFormat
- **类型**: `EUdpMessageFormat`
- **描述**: The format used to serialize the UDP message payload.

### MulticastTimeToLive
- **类型**: `uint8`
- **描述**: The time-to-live (TTL) for sent multicast packets.

### StaticEndpoints
- **类型**: `TArray<FString>`
- **描述**: The IP endpoints of static devices.
...
### ExcludedEndpoints
- **类型**: `TArray<FString>`
- **描述**: List of IP addresses that are banned from communicating with this client. Wildcard IPv4 addresses can be specified by using '*' and '?'
characters.
...
- **类型**: `bool`
- **描述**: Whether to share our list of known endpoints with those endpoints.  This allows message processors to completely mesh their networks.

### EnableTunnel
- **类型**: `bool`
- **描述**: Whether the UDP tunnel is enabled.

### TunnelUnicastEndpoint
- **类型**: `FString`
- **描述**: The local IP endpoint to listen to and send packets from.
...
- **类型**: `TArray<FString>`
- **描述**: The IP endpoints of remote tunnel nodes.
