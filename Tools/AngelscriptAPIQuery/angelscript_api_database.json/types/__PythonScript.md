...
```angelscript
bool ExecutePythonCommand(FString PythonCommand)
```
...
```angelscript
bool ExecutePythonCommandEx(FString PythonCommand, FString& CommandResult, TArray<FPythonLogOutputEntry>& LogOutput, EPythonCommandExecutionMode ExecutionMode, EPythonFileExecutionScope FileExecutionScope)
```
Execute the given Python command.
@param PythonCommand The command to run. This may be literal Python code, or a file (with optional arguments) that you want to run.
