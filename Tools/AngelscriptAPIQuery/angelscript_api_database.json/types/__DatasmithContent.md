...
```angelscript
void GetAllDatasmithUserData(TSubclassOf<UObject> ObjectClass, TArray<UDatasmithAssetUserData>& OutUserData)
```
...
```angelscript
void GetAllObjectsAndValuesForKey(FName Key, TSubclassOf<UObject> ObjectClass, TArray<UObject>& OutObjects, TArray<FString>& OutValues)
```
...
```angelscript
FString GetDatasmithUserDataValueForKey(UObject Object, FName Key, bool bPartialMatchKey)
```
...
### GetDatasmithUserDataValuesForKey
```angelscript
TArray<FString> GetDatasmithUserDataValuesForKey(UObject Object, FName Key, bool bPartialMatchKey)
```
