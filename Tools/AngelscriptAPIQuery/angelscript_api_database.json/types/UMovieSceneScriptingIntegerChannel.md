...
### AddKey
```angelscript
UMovieSceneScriptingIntegerKey AddKey(FFrameNumber InTime, int NewValue, float32 SubFrame, EMovieSceneTimeUnit TimeUnit)
```
...
### EvaluateKeys
```angelscript
TArray<int> EvaluateKeys(FSequencerScriptingRange Range, FFrameRate FrameRate)
```
...
### SetDefault
```angelscript
void SetDefault(int InDefaultValue)
```
